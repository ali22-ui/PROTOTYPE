...
import { Activity, Camera, PauseCircle, PlayCircle, Users, Video } from 'lucide-react';
import { fetchEnterpriseCameraStream, getCameraWebSocketUrl } from '../services/api';

export default function EnterpriseCameraView() {
  ...
  useEffect(() => {
    loadFrame().catch((error) => console.error('Failed to load camera stream frame:', error));
  }, []);
  ...
  useEffect(() => {
    ...
    const startFallbackPolling = () => {
      ...
      fallbackTimer = setInterval(() => {
        loadFrame().catch((error) => console.error('Camera polling error:', error));
      }, 1000);
    };
    ...
    const connect = () => {
      ...
      try {
        ...
        console.error('WebSocket initialization failed, falling back to polling:', error);
        startFallbackPolling();
        return;
      }

      socket.onopen = () => {
        setConnectionMode('websocket-live');
      };
      ...
      socket.onerror = () => {
        if (connectionMode !== 'polling-fallback') {
          startFallbackPolling();
        }
      };
  ...
  if (!streamData) {
    return <div className="rounded-xl border border-slate-200 bg-white p-6">Loading live camera stream…</div>;
  }

  return (
    <div className="space-y-4">
      <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <h3 className="text-2xl font-bold tracking-tight text-slate-800">Live Camera Monitoring</h3>
        <p className="text-sm text-slate-500">Real-time WebSocket stream with automatic fallback and AI detection overlays.</p>
      </div>
      ...
      <section className="grid gap-4 xl:grid-cols-[1.4fr_1fr]">
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm font-semibold text-slate-700">{streamData.camera_name}</p>
            </div>
            <button
              type="button"
          ...
          <div className="relative h-[430px] overflow-hidden rounded-lg border border-slate-300 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-700">
            <video
              ...
              src={demoVideoSources[Math.min(videoSourceIndex, demoVideoSources.length - 1)]}
              autoPlay
              loop
              muted
              playsInline
              onError={() => setVideoSourceIndex((current) => Math.min(current + 1, demoVideoSources.length - 1))}
            />
            ...
            <div className="absolute left-3 bottom-3 rounded-md bg-black/55 px-2 py-1 text-[11px] font-semibold text-slate-100">
              {connectionMode === 'websocket-live' ? 'WebSocket Live' : connectionMode === 'polling-fallback' ? 'Polling Fallback' : 'Connecting...'}
            </div>
            <div className="absolute right-3 bottom-3 rounded-md bg-black/55 px-2 py-1 text-[11px] font-semibold text-slate-100">
              <span className="inline-flex items-center gap-1"><Video size={12} /> CCTV Presentation Feed</span>
            </div>
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            <h4 className="text-sm font-semibold text-slate-700">Stream Health</h4>
            <div className="mt-3 space-y-2 text-sm text-slate-700">
              <div className="flex items-center justify-between rounded-lg bg-slate-50 px-2.5 py-2">
                <span className="flex items-center gap-1"><Activity size={14} /> FPS</span>
                <strong>{fps}</strong>
              </div>
              <div className="flex items-center justify-between rounded-lg bg-slate-50 px-2.5 py-2">
                <span className="flex items-center gap-1"><Users size={14} /> Active Tracks</span>
                <strong>{streamData.active_tracks}</strong>
              </div>
              <div className="flex items-center justify-between rounded-lg bg-slate-50 px-2.5 py-2">
                <span>Status</span>
                <strong className={isRunning ? 'text-emerald-700' : 'text-amber-700'}>{isRunning ? 'RUNNING' : 'PAUSED'}</strong>
              </div>
            </div>
          </div>

          <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            <h4 className="text-sm font-semibold text-slate-700">Recent Detection Events (100 sample rows)</h4>
            <ul className="mt-3 max-h-[420px] space-y-1.5 overflow-y-auto pr-1 text-xs text-slate-600">
              {detections.map((line, index) => (
                <li key={`${line}-${index}`} className="rounded border border-slate-200 bg-slate-50 px-2 py-1.5">{line}</li>
              ))}
            </ul>
          </div>
        </div>
