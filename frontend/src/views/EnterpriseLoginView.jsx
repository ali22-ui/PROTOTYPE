...
import { fetchEnterpriseAccounts } from '../services/api';

export default function EnterpriseLoginView({ onLogin }) {
  const navigate = useNavigate();
  const [accounts, setAccounts] = useState([]);
  const [selectedId, setSelectedId] = useState('');

  const selectedAccount = accounts.find((item) => item.enterprise_id === selectedId) || accounts[0];
      ...
      <div className="grid w-full max-w-4xl overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl md:grid-cols-[320px_1fr]">
        <div className="flex flex-col justify-between bg-slate-900 p-8 text-white">
          <div>
            ...
            <h1 className="text-2xl font-bold leading-tight">{selectedAccount?.company_name || 'Enterprise'} Portal</h1>
            <p className="mt-2 text-sm text-slate-200">Tourism Analytics Access</p>
          ...
          <div className="rounded-xl border border-white/20 bg-white/5 p-4 text-xs text-slate-200">
            Separate enterprise system for AI analytics and month-end report submission to linked LGU account.
          </div>
        </div>

        <form onSubmit={handleSubmit} className="p-8 md:p-10">
          <h2 className="text-xl font-bold text-slate-800">Enterprise Sign In</h2>
          <p className="mt-2 text-sm text-slate-500">Use enterprise credentials to access your reporting dashboard.</p>

          <label className="mt-6 block text-sm font-medium text-slate-700">Select Enterprise Account</label>
          <select
            value={selectedId}
          ...
          <label className="mt-4 block text-sm font-medium text-slate-700">Enterprise Username</label>
