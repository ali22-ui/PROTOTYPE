...
import { fetchEnterpriseAnalytics } from '../services/api';

const pieColors = ['#1e3a8a', '#1d4ed8', '#f4b400'];

export default function EnterpriseAnalyticsView() {
  ...
  if (!data) {
    return <div className="rounded-xl border border-slate-200 bg-white p-6">Loading enterprise analytics…</div>;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <div>
          <h3 className="text-[26px] font-semibold tracking-tight">{enterprise?.name || 'Enterprise'} - Analytics</h3>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <h4 className="mb-3 text-sm font-semibold">Demographics (Male vs Female)</h4>
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={analytics.demographics} dataKey="value" nameKey="name" outerRadius={90} label>
        ...
        <div className="h-72">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={analytics.visitorTrends}>
              ...
              <YAxis />
              <Tooltip contentStyle={{ borderRadius: 12, borderColor: '#cbd5e1' }} />
              <Line type="monotone" dataKey="visitors" stroke="#1d4ed8" strokeWidth={3} dot={{ r: 4, fill: '#f4b400' }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
          ...
          <table className="min-w-full text-left text-sm">
            ...
            <tbody>
              {analytics.reportHistory.map((report) => (
                <tr key={`${report.date}-${report.type}`} className="border-t border-slate-100">
