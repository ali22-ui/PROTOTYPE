...
export default function CameraMonitoringView(): JSX.Element {
  ...
  const hasDetectionActivity = layoutData.streamHealth.activeTracks > 0 || layoutData.events.length > 0;
  ...
  return (
    <div className="space-y-4">
      <header className="rounded-xl bg-brand-bg px-6 py-4 shadow-md">
        <h2 className="text-2xl font-bold tracking-tight text-brand-dark md:text-3xl">ARCHIES PORTAL LIVE CAMERA MONITORING</h2>
        <p className="text-sm text-brand-dark/80 md:text-base">Real-time WebSocket stream with AI detection overlays and automatic fallback monitoring.</p>
      </header>
      ...
      <section className="grid gap-4 lg:grid-cols-12">
        <article className="lg:col-span-8 rounded-xl bg-white p-4 shadow-md">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
            <div>
              <p className="text-sm font-semibold uppercase tracking-wide text-brand-dark">Camera Preview</p>
              <p className="text-xs text-brand-dark/75">{layoutData.cameraTitle}</p>
            </div>
          ...
          <article className="rounded-xl bg-white p-4 shadow-md">
            ...
            <div className="mt-3 space-y-2 text-sm text-brand-dark">
              <div className="flex items-center justify-between">
                <span>Camera</span>
                <span className="font-semibold">{isCameraRunning ? 'Connected' : 'Disconnected'}</span>
              </div>
              <div className="flex items-center justify-between">
                <span>Person Detection</span>
                <span className="font-semibold">{hasDetectionActivity ? 'Running' : 'Idle'}</span>
              </div>
              <div className="flex items-center justify-between">
                <span>Gender Classification</span>
                <span className="font-semibold">{hasDetectionActivity ? 'Ready' : 'Not loaded'}</span>
              </div>
              <div className="flex items-center justify-between">
                <span>Deduplication</span>
                <span className="font-semibold">{hasDetectionActivity ? 'Active' : 'Not initialized'}</span>
              </div>
              <div className="flex items-center justify-between">
      ...
      <section className="rounded-xl bg-white p-4 shadow-md">
        <div className="mb-2 flex items-center justify-between">
          <p className="text-base font-semibold text-brand-dark md:text-lg">Detection Log & Recent Events</p>
          ...
          <div className="max-h-[360px] overflow-y-auto">
            <table className="min-w-full text-sm">
              <thead className="sticky top-0 z-20 bg-brand-dark/90 text-left text-xs uppercase tracking-wide text-white shadow-sm backdrop-blur-sm">
                <tr>
                  <th className="bg-brand-dark/90 px-3 py-3 leading-tight">#</th>
                  <th className="bg-brand-dark/90 px-3 py-3 leading-tight">Time</th>
                  <th className="bg-brand-dark/90 px-3 py-3 leading-tight">Frame</th>
                  <th className="bg-brand-dark/90 px-3 py-3 leading-tight">Detection Details</th>
                </tr>
              </thead>
              <tbody className="bg-white">
                {visibleEvents.map((row, index) => (
                  <tr key={row.id} className="border-t border-brand-mid/25 align-top transition-colors hover:bg-brand-cream/50">
                    <td className="px-3 py-2.5 text-brand-dark/80">{index + 1}</td>
                    <td className="whitespace-nowrap px-3 py-2.5 text-brand-dark">{row.timeLabel}</td>
                    <td className="px-3 py-2.5 font-semibold text-brand-dark">{row.frame}</td>
                    <td className="px-3 py-2.5 text-brand-dark/85">{row.details}</td>
                  </tr>
                ))}
              </tbody>
            </table>
