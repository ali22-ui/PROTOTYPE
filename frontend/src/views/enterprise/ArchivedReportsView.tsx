...
export default function ArchivedReportsView(): JSX.Element {
  ...
  const loadArchivedReports = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : 'Unable to load archived reports.';
      setError(message);
    } finally {
      setLoading(false);
    }
  ...
  if (error) {
    return (
      <ErrorState message={error} onRetry={() => void loadArchivedReports()} />
    );
  }
              ...
              <tbody className="bg-white">
                {paginatedReports.length === 0 ? (
                  <tr>
                    <td
                      colSpan={6}
                      className="px-4 py-6 text-center text-sm text-brand-dark/70"
                    >
                  ...
                  paginatedReports.map((report) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-2 text-brand-dark">
                        {report.submittedDateLabel}
                      </td>
                      <td className="px-4 py-2">
                        <span
                          className={`rounded-full px-2 py-0.5 text-xs font-semibold ${statusClasses[report.lguStatus]}`}
                        >
                          [{report.lguStatus}]
                        </span>
                      </td>
