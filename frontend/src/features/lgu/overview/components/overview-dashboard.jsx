...
import { fetchOverview, fetchBarangays, fetchLogs } from '../api/get-overview';
      ...
      <section className="grid gap-3 lg:grid-cols-4">
        {metricCards.map((card) => {
          ...
          return (
            <div
              key={card.title}
              className="min-h-[136px] rounded-xl border border-slate-200 bg-white p-3.5 shadow-sm"
            >
              ...
              <div className="mt-2 flex items-end justify-between gap-2">
                <p className="text-4xl font-bold leading-none text-primary-900">
              ...
              <div className="mt-2 h-9">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart
                    data={card.trend.map((value, index) => ({ value, index }))}
                  >
                    <Area
                      type="monotone"
                      dataKey="value"
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <p className="mb-3 mt-1 text-sm text-slate-500">
              Heatmap anchored to barangay centers within San Pedro City, Laguna
              4023.
            </p>
            <CityMap
                ...
                <tbody>
                  {(overview.recentActivities || []).map((activity, index) => (
                    <tr key={activity} className="border-t border-slate-100">
                      <td className="py-1.5 font-semibold text-primary-900">
                        E-{1023 + index * 17}
                      </td>
                      <td className="py-1.5">
                        {index % 2 === 0 ? 'Visitor' : 'Tourist'}
                      </td>
                      <td className="py-1.5">
                        {index % 2 === 0 ? '45 mins' : '1 hr 30 mins'}
                      </td>
                      <td className="py-1.5">
                        <span
                          className={`rounded-full px-2 py-1 text-xs font-semibold ${index % 2 === 0 ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}
                        >
                ...
                <BarChart data={visitsToday}>
                  <Bar dataKey="a" radius={[4, 4, 0, 0]}>
                    {visitsToday.map((_, index) => (
                      <Cell
                        key={`a-${index}`}
                        fill={peakColors[index % peakColors.length]}
                      />
                    ))}
                  </Bar>
