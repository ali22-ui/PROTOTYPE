...
import { fetchEnterprises } from '../api/get-enterprises';
...
export default function EnterprisesList() {
  ...
  const statusStats = useMemo(() => {
    ...
    const renewal = rows.filter(
      (entry) => entry.status === 'Pending Renewal',
    ).length;
      ...
      <div className="grid gap-3 md:grid-cols-4">
        <div className="min-h-[122px] rounded-xl border border-slate-200 bg-white p-4 shadow-sm md:col-span-2">
          ...
          <p className="text-sm text-slate-600">
            Total establishments city-wide
          </p>
        </div>
        ...
        <div className="min-h-[122px] rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <p className="mt-2 text-2xl font-bold text-amber-700">
            {statusStats.review + statusStats.renewal}
          </p>
        </div>
      </div>
