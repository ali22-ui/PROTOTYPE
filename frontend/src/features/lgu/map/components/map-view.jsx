...
import { fetchBarangayEnterprises, fetchBarangays } from '../api/get-barangays';
    ...
    <div className="grid items-start gap-4 xl:grid-cols-[1fr_360px]">
      <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="mb-2">
          ...
          <p className="text-sm text-slate-500">
            Heatmap-only view anchored to official barangay coordinate points.
          </p>
        </div>
        <CityMap
        ...
        {selectedBarangay && (
          <div className="mt-3 grid gap-2 sm:grid-cols-3">
            ...
            <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5">
              ...
              <p className="mt-1 text-sm font-semibold text-slate-800">
                {selectedBarangay.enterpriseCount || enterprises.length}
              </p>
            </div>
            <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5">
              <p className="text-[11px] uppercase tracking-wide text-slate-500">
              ...
              <p className="mt-1 text-sm font-semibold text-primary-700">
                Pick a barangay from the right panel
              </p>
            </div>
          </div>
        )}
      ...
      <aside className="sticky top-4 self-start rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div className="mt-3 rounded-lg border border-slate-200 bg-slate-50 p-2.5">
          <p className="text-[11px] uppercase tracking-wide text-slate-500">
            Selected Barangay Enterprises
          </p>
          {selectedBarangay ? (
            enterprises.length === 0 ? (
              <p className="mt-2 text-sm text-slate-500">
                No enterprise records for this barangay.
              </p>
            ) : (
                  ...
                  >
                    ...
                    <div className="mt-1 flex items-center justify-between gap-2">
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          navigate(`/app/enterprise/${enterprise.id}`)
                        }
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-slate-500">
                    {barangay.enterpriseCount || 0} biz
                  </span>
                </div>
              </button>
            ))}
