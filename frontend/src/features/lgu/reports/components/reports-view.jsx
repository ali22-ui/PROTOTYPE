...
export default function ReportsView() {
  ...
  useEffect(() => {
    ...
    const startFallback = () => {
      ...
      fallbackTimer = setInterval(() => {
        pollFrame().catch((error) =>
          console.error('LGU camera fallback polling failed:', error),
        );
      }, 1000);
    };
    ...
    pollFrame().catch((error) =>
      console.error('LGU camera initial load failed:', error),
    );
    connect();
  ...
  const submissionSummary = useMemo(() => {
    const submitted = accounts.filter(
      (item) => item.has_submitted_for_period,
    ).length;
    const open = accounts.filter(
      (item) => item.reporting_window_status === 'OPEN',
    ).length;
  ...
  const chartData = useMemo(() => {
    ...
    return accounts
      .map((item) => ({
        ...
        score: item.has_submitted_for_period
          ? 100
          : item.reporting_window_status === 'OPEN'
            ? 60
            : 20,
      }))
      .sort(
        (a, b) => b.score - a.score || a.enterprise.localeCompare(b.enterprise),
      )
      .slice(0, 12);
  }, [accounts]);
  ...
  const filteredAccounts = accounts.filter((item) => {
    ...
    if (statusFilter === 'Closed')
      return item.reporting_window_status === 'CLOSED';
    return true;
  });
  ...
  const handleNotifyAll = async () => {
    ...
    try {
      ...
      setFeedback(
        error?.response?.data?.detail || 'Failed to notify all enterprises.',
      );
    } finally {
  ...
  const handleCloseAll = async () => {
    ...
    try {
      ...
      setFeedback(
        error?.response?.data?.detail || 'Failed to close reporting windows.',
      );
    } finally {
      setIsActionLoading(false);
    }
  ...
  const handleGenerateAuthorityPackage = async () => {
    ...
    setModalFeedback('');
    try {
      const packageData = await generateAuthorityPackage(
      ...
      setModalFeedback(
        error?.response?.data?.detail ||
          'Failed to generate authority package.',
      );
    }
  };
  ...
  const handleDownloadAuthorityPdf = async () => {
    ...
    try {
      ...
      setModalFeedback(
        error?.response?.data?.detail || 'Authority PDF download failed.',
      );
    }
  };
  ...
  const handleDownloadAuthorityDocx = async () => {
    ...
    setModalFeedback('');
    try {
      const payload = await exportAuthorityPackageDocx(
        selectedReport.report_id,
      );
      triggerDownload(payload);
      setModalFeedback(`Downloaded: ${payload.filename}`);
      ...
      setModalFeedback(
        error?.response?.data?.detail || 'Authority DOCX download failed.',
      );
    }
  };
  ...
  const pieData = useMemo(() => {
    ...
    return Object.entries(
      selectedReportDetail.charts.visitor_residence_breakdown,
    ).map(([name, value]) => ({ name, value }));
  }, [selectedReportDetail]);

  const dailySummary = selectedReportDetail?.charts?.daily_summary || [];
  const detailedRows =
    selectedReportDetail?.charts?.detailed_detection_rows || [];
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-slate-500">
              LGU controls enterprise monthly submission windows and reviews
              complete report packs.
            </p>
          </div>
        ...
        {feedback ? (
          <p className="mt-2 text-xs text-primary-700">{feedback}</p>
        ) : null}
      </div>

      <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <SummaryCard
          label="Total Enterprise Accounts"
          value={accounts.length}
        />
        <SummaryCard
          label="Submitted for Period"
          value={submissionSummary.submitted}
          accent="text-emerald-700"
        />
        <SummaryCard
          label="Window OPEN"
          value={submissionSummary.open}
          accent="text-blue-700"
        />
        <SummaryCard
          label="Waiting Submission"
          value={submissionSummary.waiting}
      ...
      <section className="grid items-start gap-4 xl:grid-cols-[1fr_1.1fr]">
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <h4 className="mb-3 text-sm font-semibold">
            Enterprise Submission Readiness
          </h4>
          <div className="h-80">
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div className="space-y-2 text-sm">
          {reports.map((item) => (
            ...
            >
              <div>
                ...
                <p className="text-xs text-slate-500">
                  {item.enterprise_name} | {item.period?.month} |{' '}
                  {item.submitted_at}
                </p>
              </div>
              <button
          ...
          {reports.length === 0 ? (
            <p className="text-xs text-slate-500">
              No report packs submitted for selected period.
            </p>
          ) : null}
        </div>
      </section>
              ...
              <>
                <div className="mt-4 grid gap-3 text-sm text-slate-700 sm:grid-cols-3">
                  <div className="rounded-lg bg-slate-50 p-3">
                    ...
                    {selectedReportDetail.period?.month || 'N/A'}
                  </div>
                  <div className="rounded-lg bg-slate-50 p-3">
                    <strong>Total Visitors:</strong>{' '}
                    {selectedReportDetail.kpis?.total_visitors_mtd || 'N/A'}
                  </div>
                  <div className="rounded-lg bg-slate-50 p-3">
                    <strong>Average Dwell:</strong>{' '}
                    {selectedReportDetail.kpis?.avg_dwell || 'N/A'}
                  </div>
                </div>
                ...
                <div className="mt-4 rounded-xl border border-slate-200 bg-white p-3">
                  <h6 className="text-sm font-semibold text-slate-700">
                    Detailed Detection Records ({detailedRows.length} rows)
                  </h6>
                    ...
                    <table className="min-w-full text-left text-xs">
                      ...
                      <tbody>
                        {detailedRows.slice(0, 200).map((row, index) => (
                          <tr
                            key={`${row.date}-${row.time_slot}-${index}`}
                          ...
                          >
                            ...
                            <td className="px-2 py-1.5">
                              {row.male_total + row.female_total}
                            </td>
                          </tr>
                        ))}
                      </tbody>
                ...
                {authorityPackage ? (
                  <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-900">
                    ...
                    <p className="mt-1">
                      Package ID: {authorityPackage.authority_package_id}
                    </p>
