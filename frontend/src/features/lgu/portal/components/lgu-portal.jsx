    ...
    <div className="space-y-4">
      <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <p className="text-sm text-slate-500">
          Aggregate view of linked enterprise reports and reporting windows.
        </p>
      </div>
      ...
      <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        <KpiCard
          label="Linked Enterprises"
          value={overview.total_linked_enterprises}
        />
        <KpiCard
          label="Submitted Reports (Current Period)"
          value={overview.submitted_reports_current_period}
        />
        <KpiCard
          label="Submission Completion Rate"
          value={`${overview.submission_completion_rate_pct}%`}
        />
      </section>
      ...
      <section className="grid gap-4 xl:grid-cols-[1.2fr_1fr]">
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="mt-3 grid gap-2 text-sm text-slate-700 sm:grid-cols-2">
            <div className="rounded-lg bg-slate-50 p-2.5">
              ...
              {overview.active_reporting_window.enterprise_id}
            </div>
            <div className="rounded-lg bg-slate-50 p-2.5">
              <strong>Period:</strong> {overview.active_reporting_window.period}
            </div>
            <div className="rounded-lg bg-slate-50 p-2.5">
              <strong>Status:</strong> {overview.active_reporting_window.status}
            </div>
            <div className="rounded-lg bg-slate-50 p-2.5">
              <strong>Opened By:</strong>{' '}
              {overview.active_reporting_window.opened_by}
            </div>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="mt-3 rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm text-slate-700">
            <p>
              <strong>Enterprise:</strong> Archies
            </p>
            <p>
              <strong>Enterprise ID:</strong> ent_archies_001
            </p>
            <p>
              <strong>LGU ID:</strong> lgu_san_pedro_001
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div className="mt-3 space-y-2 text-sm">
            ...
            >
              <div>
                ...
                <p className="text-xs text-slate-500">
                  {report.enterprise_name} | {report.period?.month} |{' '}
                  {report.submitted_at}
                </p>
              </div>
              <div className="flex items-center gap-2">
          ...
          {reports.length === 0 ? (
            <p className="text-xs text-slate-500">No reports available yet.</p>
          ) : null}
        </div>
      </section>
      ...
      {selectedReport ? (
        <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex items-center justify-between gap-3">
            <h4 className="text-sm font-semibold text-slate-700">
              Report Detail: {selectedReport.report_id}
            </h4>
            <button
              type="button"
          ...
          <div className="mt-3 grid gap-2 text-sm text-slate-700 sm:grid-cols-2">
            <div className="rounded-lg bg-slate-50 p-2.5">
              <strong>Enterprise:</strong> {selectedReport.enterprise_name}
            </div>
            <div className="rounded-lg bg-slate-50 p-2.5">
              <strong>Period:</strong> {selectedReport.period.month}
            </div>
            <div className="rounded-lg bg-slate-50 p-2.5">
              <strong>Total Visitors:</strong>{' '}
              {selectedReport.kpis.total_visitors_mtd}
            </div>
            <div className="rounded-lg bg-slate-50 p-2.5">
              <strong>Average Dwell:</strong> {selectedReport.kpis.avg_dwell}
            </div>
          </div>
