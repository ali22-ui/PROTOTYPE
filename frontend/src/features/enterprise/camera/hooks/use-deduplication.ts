...
import {
  ...
  type AppearanceFeatures,
} from './appearance-features';
import { useFaceEmbedding } from './use-face-embedding';
import { useIdentityRegistry, ReIdMethod, type ReIdMethodValue } from './use-identity-registry';
import type {
  BBoxPercent,
  BoundingBoxPixels,
  DeduplicationStats,
...
import type { KalmanFilterState } from './kalman-filter';
...
export type TrackStatusValue = (typeof TrackStatus)[keyof typeof TrackStatus];
...
export interface DeduplicationOptions {
  ...
  maxVelocityAdjustment?: number;
  dormantTimeMs?: number;
  lostTimeMs?: number;
  maxDormantTracks?: number;
  enableAppearance?: boolean;
  appearanceThreshold?: number;
  appearanceUpdateInterval?: number;
  enableFaceEmbedding?: boolean;
  faceExtractionInterval?: number;
  iouDuplicateThreshold?: number;
  maxTracksPerFrame?: number;
}
 ...
 */
export function useDeduplication(options: DeduplicationOptions = {}) {
  const config = useMemo(() => ({ ...DEFAULT_CONFIG, ...options }), [options]);

  // Track storage
  const tracksRef = useRef<Map<string, EnhancedTrack>>(new Map()); // trackId -> EnhancedTrack
   ...
   */
  const getAdaptiveThreshold = useCallback((track: EnhancedTrack): number => {
   ...
   */
  const findGeometricMatch = useCallback((detection: RawPersonDetection, activeTracks: EnhancedTrack[]) => {
    ...
    let bestMatch: EnhancedTrack | null = null;
    let bestScore = 0;
   ...
   */
  const findDormantMatch = useCallback((
    _detection: RawPersonDetection,
    dormantTracks: EnhancedTrack[],
    features: { appearance: AppearanceFeatures | null; faceEmbedding: number[] | null },
  ) => {
    let bestMatch: EnhancedTrack | null = null;
    let bestScore = 0;
    ...
    return bestMatch
      ? { track: bestMatch, score: bestScore, method: bestMethod }
      : null;
   ...
   *
   * @param detection - Detection in either format
   * @param videoElement - Video element for dimension conversion
   * @returns Normalized detection or null if invalid
   */
  const normalizeDetection = useCallback((
    detection: RawPersonDetection | PersonTrackDetection,
    ...
    if ('boundingBox' in detection && typeof detection.boundingBox.originX === 'number') {
      return detection;
    }
   ...
   * @param rawDetections - Detections from MediaPipe or usePersonDetection
   * @param videoElement - Video element for feature extraction
   * @returns Processed detections with tracking info
   */
  const processFrame = useCallback(async (
    rawDetections: Array<RawPersonDetection | PersonTrackDetection>,
    videoElement: HTMLVideoElement,
    ...
    const normalizedDetections = (rawDetections || [])
      ...
      .filter((d): d is RawPersonDetection => d !== null);
    ...
    for (const detection of filteredDetections) {
      ...
      const bboxPercent = videoElement
        ? {
          x: (detection.boundingBox.originX / (videoElement.videoWidth || 1)) * 100,
          y: (detection.boundingBox.originY / (videoElement.videoHeight || 1)) * 100,
          w: (detection.boundingBox.width / (videoElement.videoWidth || 1)) * 100,
          h: (detection.boundingBox.height / (videoElement.videoHeight || 1)) * 100,
        }
        : null;
      ...
      if (geometricMatch) {
        ...
        if (
          ...
          if (newAppearance) {
            ...
            track.appearanceUpdateCount += 1;
          }
        }

        // Extract face embedding periodically
        if (
          config.enableFaceEmbedding
      ...
      if (!matched) {
        ...
        const features: { appearance: AppearanceFeatures | null; faceEmbedding: number[] | null } = {
        ...
        if (!reIdentified) {
          ...
          const trackId = detection._trackId
    ...
    for (const track of tracks.values()) {
      if (!activeTrackIds.has(track.trackId)) {
        ...
        if (track.status === TrackStatus.ACTIVE) {
          if (track.missedFrames * 150 > config.dormantTimeMs) {
            // Assuming ~150ms per frame
    ...
    setStats((prev) => ({
      ...
      dormantTracks: Array.from(tracks.values()).filter((t) => t.status === TrackStatus.DORMANT)
        .length,
      ...
      reIdentificationCount: identityRegistry.stats.reIdentificationCount,
    }));
   ...
   */
  const getActiveDetections = useCallback((): Array<EnhancedTrack & { dwellSeconds: number }> => {
