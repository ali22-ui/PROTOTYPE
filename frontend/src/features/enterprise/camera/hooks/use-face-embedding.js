...
export function useFaceEmbedding(options = {}) {
   ...
   * @param {object} bbox - Person bounding box {x, y, w, h} in percentage
   * @param {string} trackId - Track ID for caching
   * @returns {object|null} Face data {embedding, confidence, landmarks} or null
   */
  const extractFaceEmbedding = useCallback(async (videoElement, bbox, trackId) => {
    ...
    try {
      ...
      ctx.drawImage(
        videoElement,
        pixelX, faceRegionY, pixelW, faceRegionH,
        0, 0, pixelW, faceRegionH
      );
   ...
   * @param {Array} emb1 - First 128-D embedding
   * @param {Array} emb2 - Second 128-D embedding
   * @returns {number} Euclidean distance (lower = more similar)
   */
  const euclideanDistance = useCallback((emb1, emb2) => {
   ...
   * @param {Array} embedding - 128-D embedding to match
   * @param {Array} storedEmbeddings - Array of {personId, embedding, ...}
   * @returns {object|null} Best match {personId, distance, confidence} or null
   */
  const matchEmbedding = useCallback((embedding, storedEmbeddings) => {
   ...
   * @param {Array} detections - Array of detections with {trackId, bbox}
   * @returns {Map} Map of trackId -> faceData
   */
  const extractBatch = useCallback(async (videoElement, detections) => {
   ...
   */
  const getCachedEmbedding = useCallback((trackId) => {
    return embeddingCacheRef.current.get(trackId) || null;
  }, []);

  /**
   * Clear cached embedding for a track.
   * @param {string} trackId - Track ID to clear
   */
   ...
   * @returns {Array} Array of {trackId, ...faceData}
   */
  const getAllCachedEmbeddings = useCallback(() => {
