...
export default function EnterpriseDashboard() {
  ...
  const residencePieData = useMemo(() => {
    ...
    return Object.entries(dashboard.visitor_residence_breakdown).map(
    ...
    try {
      ...
      if (action === 'maintenance') {
        const response = await requestMaintenance({
          enterprise_id: profile.enterprise_id,
          message: 'Camera calibration requested.',
        });
        setActionFeedback(
          `${response.message} Ticket: ${response.ticket.ticket_id}`,
        );
        return;
      }
    ...
    <div className="space-y-4">
        ...
        <div className="flex items-center justify-between gap-4">
          <div>
            <h3 className="text-2xl font-bold tracking-tight text-slate-800">
              {dashboard.header.company_name}
            </h3>
            <p className="text-sm text-slate-500">
              {dashboard.header.datetime_label}
            </p>
          </div>
      ...
      <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard
          ...
          value={dashboard.key_stats.average_dwell_time}
          subValue="Across all detections"
        />
      </section>
            ...
            <div className="h-[330px]">
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={trafficTrend}>
                  ...
                  <Area
                    type="monotone"
                    dataKey="female_total"
                    stroke={trendColors[1]}
                    fill="rgba(244,180,0,.18)"
                    strokeWidth={2}
                  />
                  <Line
                    type="monotone"
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <p className="mt-2 text-sm text-slate-600">
              Use the dedicated <strong>Camera Monitoring</strong> tab for live
              simulation.
            </p>
            <div className="mt-3 rounded-lg border border-slate-200 bg-slate-50 p-3 text-xs text-slate-600">
              <p className="flex items-center gap-1">
                <Video size={13} /> CCTV Status: ACTIVE (Main Entrance - Camera
                1)
              </p>
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div className="mt-3 grid gap-2">
              ...
              <ActionButton
                icon={<Hammer size={14} />}
                label="Submit Manual Log Correction"
                onClick={() => handleAction('manual-correction')}
              />
              <button
                type="button"
                disabled={!reportingOpen || isSubmitting}
                onClick={handleSubmitToLgu}
                className={`flex items-center justify-center gap-2 rounded-lg px-3 py-2 text-sm font-semibold text-white ${reportingOpen && !isSubmitting ? 'bg-primary-500 hover:bg-primary-700' : 'cursor-not-allowed bg-slate-400'}`}
              >
                <Send size={14} />
            ...
            <p className="mt-2 text-xs text-slate-500">
              Reporting window for {windowState.period}:{' '}
              <strong>{windowState.status}</strong>
            </p>
            {actionFeedback ? (
              <p className="mt-2 text-xs text-primary-700">{actionFeedback}</p>
            ) : null}
          </div>

          <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            <h4 className="text-sm font-semibold text-slate-700">
              Visitor Pulse (Last 12 slots)
            </h4>
            <div className="h-44">
              <ResponsiveContainer width="100%" height="100%">
