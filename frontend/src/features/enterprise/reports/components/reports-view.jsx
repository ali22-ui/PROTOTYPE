...
import {
  ...
  fetchEnterpriseReportHistory,
  fetchReportingWindowStatus,
  submitEnterpriseReport,
} from '../api/submit-report';
    ...
    <div className="space-y-4">
      <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <p className="text-sm text-slate-500">
          Prepare and submit monthly report packs to LGU when reporting window
          is open.
        </p>
      </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="mt-3 space-y-2 text-sm text-slate-700">
            <div className="rounded-lg bg-slate-50 p-2.5">
              <strong>Enterprise:</strong> {profile.company_name}
            </div>
            <div className="rounded-lg bg-slate-50 p-2.5">
              <strong>Period:</strong> {windowState.period}
            </div>
            <div className="rounded-lg bg-slate-50 p-2.5">
              <strong>Status:</strong> {windowState.status}
            </div>
          </div>
          ...
          <button
            ...
            className={`mt-3 w-full rounded-lg px-3 py-2 text-sm font-semibold text-white ${canSubmit && !isSubmitting ? 'bg-primary-500 hover:bg-primary-700' : 'cursor-not-allowed bg-slate-400'}`}
          >
            Submit Monthly Report to LGU
          </button>
          <p className="mt-2 text-xs text-slate-500">
            Submit button remains disabled until LGU opens reporting window.
          </p>
          {feedback ? (
              ...
              history.map((item) => (
                ...
                >
                  ...
                  <p className="text-xs text-slate-500">
                    {item.period?.month} | {item.submitted_at}
                  </p>
                  <div className="mt-1 flex items-center gap-2 text-xs text-slate-700">
                    <Clock3 size={12} />
                    <span>
                      Window status at submit:{' '}
                      {item.audit?.reporting_window_status_at_submit || 'N/A'}
                    </span>
                  </div>
                  <div className="mt-1 flex items-center gap-2 text-xs text-emerald-700">
                    <FileCheck2 size={12} />
                    <span>
                      Total Visitors: {item.kpis?.total_visitors_mtd ?? 'N/A'}
                    </span>
                  </div>
                </div>
              ))
